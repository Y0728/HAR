...
class Window(QDialog):
    def __init__(self, parent=None, size=[]):
        ...
        self.Gradients = OrderedDict([
            ...
            ('hot', {'ticks': [(0.3333, (185, 0, 0, 255)), (0.6666, (255, 220, 0, 255)), (1, (255, 255, 255, 255)),
                               (0, (0, 0, 0, 255))], 'mode': 'rgb'}),
            ('jet', {'ticks': [(1, (166, 0, 0, 255)), (0.32247191011235954, (0, 255, 255, 255)),
                               (0.11348314606741573, (0, 68, 255, 255)), (0.6797752808988764, (255, 255, 0, 255)),
    ...
    def setConnectionLayout(self):
        ...
        self.configType.addItems(["3D People Counting", "SDK Out of Box Demo", "Long Range People Detection",
                                  "Sense and Detect HVAC Control", "(Legacy) 2D People Counting",
                                  "(Legacy): Overhead People Counting", "Capon3DAOP", "Replay"])
    ...
    def drawBoundaryBox3d(self, index):
        ...
        self.bottomSquare[index].setData(pos=squareLine, color=pg.glColor('b'), width=2, antialias=True,
                                         mode='line_strip')
    ...
    def heightPlots(self):
        ...
        yMin = -3
        yMax = 3
        self.targetSize = np.zeros((7, 20,
                                    100))  # 100 frames of data, 20 tracks, height, relHeight, length, width, age, average height, height delta
    ...
    def dataBtnClicked(self, btn):
        ...
        if btn.isChecked():
            ...
            self.targetInfoPersistent.to_csv(type_text + timestr + name + '-target-' + str(len(self.targetInfoPersistent)) + ".csv",
                                            index=0, header=1)
            self.pointInfoPersistent.to_csv(type_text + timestr + name + '-point-' + str(len(self.targetInfoPersistent)) + ".csv",
                                            index=0, header=1)
    ...
    def plot3DQTGraph(self):
        ...
        if (sphereDebug == 1):
            ...
            self.sphere = gl.GLMeshItem(vertexes=sphereTrigs, smooth=False, drawEdges=True, edgeColor=pg.glColor('w'),
                                        drawFaces=False)
        ...
        self.evmBox = gl.GLMeshItem(vertexes=verts, smooth=False, drawEdges=True, edgeColor=pg.glColor('r'),
                                    drawFaces=False)
    ...
    def updateGraph(self, parsedData):
        ...
        if (
                self.configType.currentText() == '3D People Counting' or self.configType.currentText() == 'Capon3DAOP' or self.configType.currentText() == 'Sense and Detect HVAC Control'):
        ...
        if (self.graphFin):
            ...
            if (self.threeD):
                ...
                self.get_thread = updateQTTargetThread3D(pointIn, targets, indicesIn, self.scatter, self.pcplot,
                                                         numTargets, self.ellipsoids, self.coordStr, classifierOutput,
                                                         self.zRange, self.gw, self.plotByIndex.isChecked(),
                                                         self.plotTracks.isChecked(), self.bbox,
                ...
                npc = pointIn[0:2, :]
                print(np.shape(npc))
                self.legacyThread = update2DQTGraphThread(npc, targets, numTargets, indexes, numPoints, self.trailData,
                                                          self.activeTrail, self.trails, self.scatter2D,
                                                          self.gatingScatter)
